<div class="class_header_wrapper">
    <div class="class_header_container">
        <p class="class_header_title"><a href="../views/class_whiteboard.php">Year 11, Set B2 ICT (IB211)</a></p>
        <div class="class_nav_wrapper">
            <a href="../views/class_whiteboard.php" class="class_nav_button rectangle_fill_button class_nav_button_active">Whiteboard</a>
            <a href="../views/class_topics.php" class="class_nav_button rectangle_fill_button">Topics</a>
            <div class="class_nav_action_button_wrapper drop_down_button_wrapper">
                    <button class="fill_button_wrapper"><span>Create</span><i class="fas fa-angle-down"></i></button>
                    <div class="drop_down_options_wrapper" id="class_header_dropdown">
                        <a class="drop_down_option topic_create_button">Topic</a>
                        <a class="drop_down_option create_reminder_button">Reminder</a>
                    </div> <!-- Drop down options wrapper -->
                </div> <!-- Drop down button wrapper -->
        </div> <!-- Class nav wrapper-->
    </div> <!-- Class header container -->
</div> <!-- Class header wrapper -->